...
class ContentController extends AppController
{
    ...
    public function index(){
        ...
        if($this->request->getQuery('brand') or $this->request->getParam('brands')){
            ...
            $brand = TableRegistry::get('Shop.ShopBrands')->find('all')
                ->where([
                    'OR'=>[
                        ['slug' => $brand],
        ...
        if($this->request->getQuery('label') or $this->request->getParam('label')){
            ...
            $label = TableRegistry::get('Shop.ShopLabels')->find('all')
                ->where([ 'title' =>$label ])
                ->first();
            if( isset($label['id']) ){
                $results->join([
            ...
            if($list_id):
                ...
                switch ( $sort ) {
                    ...
                    case 'order.desc': //پرفروش ترین
                        $query = TableRegistry::get('Shop.ShopOrderproducts')
                    ...
                    case 'popularity.desc': //محبوب ترین
                        $query = TableRegistry::get('Shop.ShopFavorites')
                            ->find('list',['keyField'=>'post_id','valueField'=>'count']); 
                        $p = $query->select(['post_id','count' => $query->func()->count('post_id')])
                            ->group(['post_id'])
                            ->enableHydration(false)
    ...
    public function Price(){
        ...
        try{
            ...
            $this->Meta =  TableRegistry::get('Shop.ShopProductMetas');
