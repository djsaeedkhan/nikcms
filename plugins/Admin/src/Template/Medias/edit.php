...
?>
<?= $this->html->css([
	'/admin/css/bootstrap-jasny.min.css',
	'/admin/css/jquery.Jcrop.min.css',
])?>

	<div class="row-fluid">
		<?php if ($step == 1): ?>
		<h1>مرحله یک: انتخاب تصویر</h1>
		<?php if(!empty($vals['errors'])): ?>
		<?php foreach($vals['errors'] as $error): ?>
		<p class="alert alert-error"><?php echo $error; ?></p>
		<?php endforeach; ?>
		<?php endif; ?>
		<form id="upload_form" enctype="multipart/form-data" method="post" action1="<?php echo $_SERVER['PHP_SELF']; ?>">
			...
			<div class="fileupload fileupload-new" data-provides="fileupload">
				...
				<div>
					...
					<span class="fileupload-exists">Change</span>
						<input type="file" name="<?php echo $fieldname; ?>" id="fileinput" />
					</span>
					<a href="#" class="btn btn-success fileupload-exists" data-dismiss="fileupload" id="remove">Remove</a>
...
<?= $this->html->script([
	//'/admin/js/test/jquery.min.js',
	'/admin/js/jquery.Jcrop.min.js',
	'/admin/js/test/bootstrap.min.js',
	'/admin/js/script.js',
],['nonce'=>get_nonce]);?>